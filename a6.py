...
class BayesClassifier:
    ...
    def train(self) -> None:
        ...
        #Trains the classifier by generating pos_freqs and neg_freqs
        #dictionaries on all files in the folder given by training_data_directory.
        ...
        self.save_dict(self.pos_freqs,self.pos_filename)
                ...
                #update freqs and neg and pos freqs to rep dictionaries passed into update
    ...
    def classify(self, text: str) -> str:
        ...
        pos_prob = 0
        neg_prob = 0
        all_pos = sum(self.pos_freqs.values())
        all_neg = sum(self.neg_freqs.values())
        

        for token in tokens:
            if token in self.pos_freqs:
                #calculate numerator and outside do math.log and add to total probabilities
                pos_prob += math.log(self.pos_freqs[token] / all_pos)
            pos_prob += 1
            if token in self.neg_freqs:
                neg_prob += math.log(self.neg_freqs[token] / all_neg)
            neg_prob += 1
        if (pos_prob > neg_prob) or (pos_prob == neg_prob):
            return "positive"
        else:
            return "negative"
